...
export class Permissions extends ContractWrapper {
  ...
  public async getService(provider: string): Promise<Service | undefined> {
    ...
    return {
      name: result.name,
      url: result.url,
      permissions: result.permissions.map((permission: any) => Permission[permission.toNumber()]),
      provider: {
        address: provider,
        countryCode: ethers.toUtf8String(result.countryCode),
        partyId: ethers.toUtf8String(result.partyId),
      },
    };
  }
   ...
   */
  public async setService(name: string, url: string, permissions: Permission[]): Promise<ethers.TransactionReceipt> {
    this.verifyWritable();
   ...
   */
  public async setServiceRaw(name: string, url: string, permissions: Permission[], signer: string): Promise<ethers.TransactionReceipt> {
    this.verifyWritable();
   ...
   */
  public async deleteService(): Promise<ethers.TransactionReceipt> {
    this.verifyWritable();
   ...
   * constructor is the "spender": they send and pay for the transaction on the network.
   */
  public async deleteServiceRaw(signer: string): Promise<ethers.TransactionReceipt> {
    this.verifyWritable();
    const wallet = new ethers.Wallet(signer);
   ...
   */
  public async createAgreement(provider: string): Promise<ethers.TransactionReceipt> {
    this.verifyWritable();
   ...
   * constructor is the "spender": they send and pay for the transaction on the network.
   */
  public async createAgreementRaw(provider: string, signer: string): Promise<ethers.TransactionReceipt> {
    this.verifyWritable();
    const wallet = new ethers.Wallet(signer);
   ...
   * @param {string} provider the address of the service provider
   */
  public async revokeAgreement(provider: string): Promise<ethers.TransactionReceipt> {
    this.verifyWritable();
   ...
   * constructor is the "spender": they send and pay for the transaction on the network.
   */
  public async revokeAgreementRaw(provider: string, signer: string): Promise<ethers.TransactionReceipt> {
    this.verifyWritable();
    const wallet = new ethers.Wallet(signer);
