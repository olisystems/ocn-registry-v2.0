...
task("execute", "Execute proposal")
  .addOptionalParam("id", "proposal id to execute", undefined, string)
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    async function execute(proposal: Proposal) {
      ...
      const executeTx = await governor.execute([await ocnPaymentManager.getAddress()], [0], [proposal.encodedFunction], descriptionHash);
      ...
      console.log(`New Funding Yearly Amount Value: ${currrentValue} at ${await ocnPaymentManager.getAddress()}`);
    }
